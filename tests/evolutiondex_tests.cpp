...
public:
	...
	vector<int64_t> total() {
		...
		int64_t total_eos = balance(N(alice), 0) + balance(N(bob), 0) + system_balance(EVO_value).at(0) + system_balance(ETUSD_value).at(0);
		int64_t total_voice = balance(N(alice), 1) + balance(N(bob), 1) + system_balance(EVO_value).at(1);
		int64_t total_tusd = balance(N(alice), 2) + balance(N(bob), 2) + system_balance(ETUSD_value).at(1);
...
BOOST_FIXTURE_TEST_CASE(memoexchange_test, evolutiondex_tester) try {
	...
	BOOST_REQUIRE_EQUAL(
		success(), transfer(N(eosio.token), N(alice), N(evolutiondex), asset::from_string("4.0000 EOS"), "exchange: EVO, 16.6535 VOICE"));
	...
	BOOST_REQUIRE_EQUAL(
		wasm_assert_msg("available is less than expected"),
		transfer(N(eosio.token), N(alice), N(evolutiondex), asset::from_string("4000.0000 TUSD"), "exchange: ETUSD, 403.5641 EOS"));
	...
	BOOST_REQUIRE_EQUAL(
		success(),
		transfer(N(eosio.token), N(alice), N(evolutiondex), asset::from_string("4000.0000 TUSD"), "exchange: ETUSD, 403.5639 EOS"));
	...
	auto old_vec = system_balance(EVO.value);
	transfer(N(eosio.token), N(alice), N(evolutiondex), asset::from_string("4.0000 EOS"), "exchange: EVO, 10000 VOICE, nothing to say");
	auto new_vec = system_balance(EVO.value);
	BOOST_REQUIRE_EQUAL(is_increasing(old_vec, new_vec), true);

	old_vec = system_balance(ETUSD.value);
	transfer(
		N(eosio.token), N(bob), N(evolutiondex), asset::from_string("300000000000000.0000 TUSD"), "exchange: ETUSD, 40000000000000 EOS,");
...
BOOST_FIXTURE_TEST_CASE(the_other_actions, evolutiondex_tester) try {
 ...
 please run openext action or write exchange details in the memo of your transfer"),
	...
	BOOST_REQUIRE_EQUAL(
		...
		transfer(N(eosio.token), N(alice), N(evolutiondex), asset::from_string("1000.0000 EOS"), "deposit to: evolutiondex"));
	BOOST_REQUIRE_EQUAL(success(), transfer(N(eosio.token), N(alice), N(evolutiondex), asset::from_string("1000.0000 EOS"), ""));
	BOOST_REQUIRE_EQUAL(success(), transfer(N(eosio.token), N(alice), N(evolutiondex), asset::from_string("20000.0000 VOICE"), ""));
