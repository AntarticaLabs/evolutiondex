#include <wevotethefee/wevotethefee.hpp>
...
void wevotethefee::updatefee(symbol_code pair_token) {
	action(permission_level{get_self(), "active"_n}, "evolutiondex"_n, "changefee"_n, make_tuple(pair_token, median(pair_token))).send();
}
...
void wevotethefee::votefee(name user, symbol_code pair_token, int fee_voted) {
	...
	if (acnt == acnts.end()) {
		...
		acnts.modify(acnt, user, [&](auto& a) { a.fee_index_voted = fee_index_voted; });
	}
...
void wevotethefee::closefeetable(symbol_code pair_token) {
	...
	auto votes = table->votes;
	auto is_empty = all_of(votes.begin(), votes.end(), [](const int& votes_number) { return votes_number == 0; });
...
void wevotethefee::addvote(symbol_code pair_token, int fee_index, int64_t amount) {
	...
	auto table = tables.find(pair_token.raw());
	check(table != tables.end(), "fee table nonexistent, run openfeetable");
	tables.modify(table, same_payer, [&](auto& a) { a.votes.at(fee_index) += amount; });
}
