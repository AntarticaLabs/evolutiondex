#include <evolutiondex/evolutiondex.hpp>
...
void evolutiondex::addliquidity(name user, asset to_buy, asset max_asset1, asset max_asset2) {
	require_auth(user);
...
void evolutiondex::remliquidity(name user, asset to_sell, asset min_asset1, asset min_asset2) {
	require_auth(user);
...
void evolutiondex::add_signed_liq(name user, asset to_add, bool is_buying, asset max_asset1, asset max_asset2) {
	...
	check((to_pay1.quantity.symbol == max_asset1.symbol) && (to_pay2.quantity.symbol == max_asset2.symbol), "incorrect symbol");
	check((to_pay1.quantity.amount <= max_asset1.amount) && (to_pay2.quantity.amount <= max_asset2.amount),
...
void evolutiondex::exchange(name user, symbol_code pair_token, extended_asset ext_asset_in, asset min_expected) {
	require_auth(user);
...
extended_asset evolutiondex::process_exch(symbol_code pair_token, extended_asset ext_asset_in, asset min_expected) {
...
void evolutiondex::memoexchange(name user, extended_asset ext_asset_in, string_view details) {
	...
	action(permission_level{get_self(), "active"_n},
		   ...
		   "transfer"_n,
		   std::make_tuple(get_self(), user, ext_asset_out.quantity, std::string(memo)))
		.send();
}

void evolutiondex::inittoken(
